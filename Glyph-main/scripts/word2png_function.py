...
def resolve_font_path(font_path):
    ...
    relative_candidates = [
        REPO_ROOT / font_path,
        REPO_ROOT / 'config' / Path(font_path).name,
        Path.cwd() / font_path,
    ]
    for probe in relative_candidates:
        if probe.exists():
...
def load_config(config_path):
    ...
    # Resolve font paths relative to the config file so callers can run from
    # the repo root, scripts/, or any other working directory.
    if 'font-path' in config and isinstance(config['font-path'], str):
        font_path = config['font-path']
        if not os.path.isabs(font_path):
            config['font-path'] = os.path.abspath(
                os.path.join(os.path.dirname(config_path), font_path)
            )
        config['font-path'] = resolve_font_path(config['font-path'])
...
def text_to_images(text, output_dir, config_path=None, config_dict=None, unique_id=None):
    ...
    if config_dict is None:
        ...
        if 'font-path' in config and isinstance(config['font-path'], str):
            ...
            config['font-path'] = resolve_font_path(None)
    ...
    newline_markup = config.get('newline-markup', '<br/>') or '<br/>'
